...
<Alert>
  ...
  <AlertTitle>Works Entirely Offline</AlertTitle>
  <AlertDescription>
    No API key required for local trace files! Only need a FlameDeck API key for analyzing remote traces.
  </AlertDescription>
</Alert>
...
<Alert>
  ...
  <AlertTitle>Automatic Format Detection</AlertTitle>
  <AlertDescription>
    The server automatically detects trace formats and handles gzipped files transparently.
  </AlertDescription>
</Alert>
...
**Parameters:**
- `trace` (string): File path or FlameDeck URL
- `sortBy` (optional): `'self'` or `'total'` time (default: `'total'`)
- `offset` (optional): Starting position for pagination (default: 0)
- `limit` (optional): Number of functions to return (default: 15)
...
- `mode` (optional): `'light'` or `'dark'` theme (default: `'light'`)
- `startTimeMs` (optional): Start time for zoomed view
- `endTimeMs` (optional): End time for zoomed view
- `startDepth` (optional): Stack depth to start visualization
...
**Parameters:**
- `trace` (string): File path or FlameDeck URL
- `frameName` (string): Exact function name to focus on
...
<Alert>
  ...
  <AlertDescription>
    Subsequent tool calls on the same trace are nearly instant thanks to smart caching.
  </AlertDescription>
</Alert>
...
<Alert>
  ...
  <AlertTitle>Large File Performance</AlertTitle>
  <AlertDescription>
    For traces larger than 100MB, consider using time-based filtering to focus on specific time ranges.
  </AlertDescription>
</Alert>
