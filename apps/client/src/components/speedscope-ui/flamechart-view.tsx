...
export const FlamechartView = memo(
  forwardRef<FlamechartViewRef, FlamechartViewContainerProps>((props, ref) => {
    ...
    const configSpaceSize = () => {
      return new Vec2(props.flamechart?.getTotalWeight() ?? 0, props.flamechart.getLayers().length);
    };
    ...
    return (
      <div
        className="w-full h-full flex flex-col left-0 top-0 relative overflow-hidden" /* ref={containerRef} */
      >
        ...
        {hoverMenu.visible && hoverMenu.node && (
          ...
          >
            <div className="px-3 py-1.5">
              <div className="truncate max-w-[300px] font-mono">{hoverMenu.node.frame.name}</div>
              ...
              <div className="text-xs flex justify-between mt-1">
                <span className="text-muted-foreground pr-3">
                  Total: {formatValue(hoverMenu.node.getTotalWeight())}
                </span>
                <span className="text-muted-foreground">
                  Self: {formatValue(hoverMenu.node.getSelfWeight())}
                </span>
              </div>
            </div>
          </ContextMenu>
