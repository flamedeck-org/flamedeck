...
const ProtectedRoute = () => {
  ...
  console.log("[ProtectedRoute] State Values:", { loading, profileLoading, user: !!user, profile: !!profile, profileUsername: profile?.username });
  ...
  if (profile && !profile.username) {
    console.log("[ProtectedRoute] User exists, profile exists, username MISSING. Redirecting to /onboarding/username.");
  ...
  console.error("[ProtectedRoute] Fallback Error: User exists but profile is missing after loading.", { user, profile, loading, profileLoading });
  return (
        ...
        <Card className="w-full max-w-md border-destructive/50 bg-destructive/5 mt-8">
            ...
            <CardContent className="text-center">
                <p className="text-sm text-muted-foreground">
                 Please check your connection or try refreshing the page. If the problem persists, contact support.
                </p>
            </CardContent>
