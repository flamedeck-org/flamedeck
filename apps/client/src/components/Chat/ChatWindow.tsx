...
export const ChatWindow = forwardRef<ChatWindowHandle, ChatWindowProps>(({
  ...
  const handleScroll = useCallback(() => {
    const viewport = internalScrollAreaRef.current?.querySelector(':scope > [data-radix-scroll-area-viewport]') as HTMLElement | null;
    if (viewport) {
  ...
  useImperativeHandle(ref, () => ({
    scrollToBottom: (force = false) => {
      ...
      if (internalScrollAreaRef.current) {
        const viewport = internalScrollAreaRef.current.querySelector(':scope > [data-radix-scroll-area-viewport]') as HTMLElement;
        if (viewport) {
            setTimeout(() => {
                const vp = internalScrollAreaRef.current?.querySelector(':scope > [data-radix-scroll-area-viewport]') as HTMLElement | null;
                if (vp) {
  ...
  return (
    <div className="fixed bottom-24 right-6 w-96 h-[32rem] bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded-lg shadow-xl flex flex-col z-40 overflow-hidden">
      ...
      <div className="p-3 border-t dark:border-gray-700 flex items-center space-x-2 bg-gray-50 dark:bg-gray-700 chat-input-area">
        ...
        />
        <Button onClick={handleSend} size="icon" disabled={isLoading || !input.trim()} aria-label="Send Message">
