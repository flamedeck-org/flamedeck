...
const Sidebar: React.FC<SidebarProps> = ({ minimized = false, mobile = false }) => {
  ...
  return (
    <TooltipProvider delayDuration={0}>
      <aside
        className={`${
          mobile ? 'w-full h-full' : minimized ? 'w-16 h-full' : 'w-64 h-full'
        ...
        <nav className="flex-1 px-2 py-6 space-y-2 overflow-y-auto">
          <NavLink
            ...
            className={({ isActive }) => {
              ...
              return `flex items-center ${minimized ? `justify-center ${MINIMIZED_BUTTON_SIZE}` : 'space-x-2 pl-3.5 pr-3 py-2 h-9'} rounded-md text-sm font-medium transition-colors ${
                isViewerActive
          ...
          <NavLink
            to="/settings"
            className={({ isActive }) =>
              `flex items-center ${minimized ? `justify-center ${MINIMIZED_BUTTON_SIZE}` : 'space-x-2 pl-3.5 pr-3 py-2 h-9'} rounded-md text-sm font-medium transition-colors ${
                isActive
                  ? 'bg-accent text-accent-foreground'
                  : 'text-muted-foreground hover:bg-accent hover:text-accent-foreground'
              }`
