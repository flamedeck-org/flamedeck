  ...
  const handleUpload = useCallback(async () => {
    ...
    try {
      ...
      if (err instanceof UploadError) {
        setError(`Upload failed (${err.status}): ${err.message}${err.details ? ` - ${JSON.stringify(err.details)}` : ''}`);
      } else {
    ...
    <div className="p-4 border rounded-lg space-y-4 max-w-md mx-auto my-6">
      ...
      {resultUrl && (
        <Alert>
          ...
          <AlertDescription>
            Trace uploaded. View at: <a href={resultUrl} target="_blank" rel="noopener noreferrer" className="font-medium text-primary underline underline-offset-4">{resultUrl}</a>
          </AlertDescription>
        </Alert>
      )}
    </div>
