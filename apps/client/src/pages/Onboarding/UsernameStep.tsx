...
function UsernameStep() {
  ...
  const updateUsernameMutation = useMutation({
    mutationFn: async (formData: UsernameFormValues) => {
      ...
      if (formData.username !== lastAvailableUsername) {
        throw new Error(
          'Username changed since availability check. Please wait for the check to complete.'
        );
      }
    ...
    onSuccess: async () => {
      try {
        ...
        toast({
          ...
          description: 'Profile updated, but failed to refresh data for navigation.',
          variant: 'destructive',
        });
  ...
  const onSubmit: SubmitHandler<UsernameFormValues> = (data) => {
    if (data.username !== lastAvailableUsername || availabilityStatus !== 'available') {
      toast({
        ...
        description: 'Please ensure the username is available before saving.',
        variant: 'destructive',
      });
      return;
    }
  ...
  const getInputOutlineClass = () => {
    if (!currentUsername || availabilityStatus === 'idle' || availabilityStatus === 'invalid')
      return ''; // Default
    ...
    if (availabilityStatus === 'taken' || availabilityStatus === 'error')
      return 'ring-2 ring-offset-0 ring-red-500'; // Taken/Error (thicker)
    return '';
  };
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center space-x-2 relative">
                ...
                <Input
                  ...
                  aria-invalid={
                    errors.username || availabilityStatus === 'taken' ? 'true' : 'false'
                  }
                />
