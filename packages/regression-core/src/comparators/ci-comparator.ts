...
import { ComparisonInput } from '../types/metrics';
import type {
  ComparisonResult,
...
export abstract class CIComparator<T extends MetricCollection> {
  ...
  private async collectMeasurements(
    ...
    const measurements = new Map<
      string,
      { base: MeasurementPoint<T>[]; treatment: MeasurementPoint<T>[] }
    >();
    ...
    for (let i = 0; i < config.iterations; i++) {
      ...
      console.log(
        `Iteration ${i + 1}/${config.iterations} (${isBaseBranch ? 'base' : 'treatment'})`
      );
  ...
  private calculateScenarioAssessment(comparisons: any[]): RegressionAssessment {
    ...
    const confidence =
      regressions.length > 0 ? Math.min(...regressions.map((r) => 1 - r.pValue)) : 0;
  ...
  private calculateSummary(scenarioResults: ScenarioComparisonResult<T>[]): ComparisonSummary {
    ...
    const mostImpactedMetrics: MetricImpact[] = Array.from(metricImpacts.entries())
      .map(([metricName, impact]) => ({
        ...
        averageEffectSize:
          impact.effectSizes.reduce((a, b) => a + b, 0) / impact.effectSizes.length,
        maxEffectSize: Math.max(...impact.effectSizes),
        severity: this.classifyMetricSeverity(
          impact.regressionCount,
          Math.max(...impact.effectSizes)
        ),
      }))
  ...
  private calculateOverallAssessment(
    scenarioResults: ScenarioComparisonResult<T>[]
  ): RegressionAssessment {
