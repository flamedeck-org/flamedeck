...
import { ResourceMetrics } from '../types/playwright-metrics';
...
export class ResourceTimingCollector {
    ...
    static async collectResourceTiming(page: Page): Promise<ResourceMetrics> {
        const resourceData = await page.evaluate(() => {
            const resourceTimingJson = JSON.stringify(
     ...
     */
    static async getDetailedResourceTiming(page: Page): Promise<Array<{
        name: string;
     ...
     */
    static async getSlowResources(page: Page, thresholdMs: number = 1000): Promise<Array<{
        name: string;
        duration: number;
        transferSize: number;
        type: string;
     ...
     */
    static async getLargeResources(page: Page, thresholdBytes: number = 500000): Promise<Array<{
        name: string;
        transferSize: number;
        encodedBodySize: number;
        type: string;
        ...
        return page.evaluate((threshold) => {
            ...
            return resourceEntries
                ...
                .map((resource: any) => {
                    ...
                    const compression = encodedSize > 0 ? ((encodedSize - transferSize) / encodedSize) * 100 : 0;
    ...
    static calculateResourceScore(metrics: ResourceMetrics): {
        ...
        if (metrics.averageResourceLoadTime > 500) {
            issues.push(`Slow average resource load time: ${metrics.averageResourceLoadTime.toFixed(0)}ms`);
            score -= 20;
        } else if (metrics.averageResourceLoadTime > 200) {
            issues.push(`Moderate average resource load time: ${metrics.averageResourceLoadTime.toFixed(0)}ms`);
            score -= 10;
        }
