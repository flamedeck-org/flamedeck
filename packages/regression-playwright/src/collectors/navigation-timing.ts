...
import { NavigationMetrics } from '../types/playwright-metrics';
...
export class NavigationTimingCollector {
    ...
    static async collectNavigationTiming(page: Page): Promise<NavigationMetrics> {
        const navigationData = await page.evaluate(() => {
            const navigationTimingJson = JSON.stringify(
        ...
        return page.evaluate(() => {
            ...
            const timestamps = {
                ...
                secureConnectionStart: nav.secureConnectionStart > 0 ?
                    nav.secureConnectionStart - nav.navigationStart : 0,
     ...
     */
    static async getServerTiming(page: Page): Promise<Array<{
        name: string;
        duration: number;
        description: string;
    }>> {
