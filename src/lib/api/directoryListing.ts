...
export async function getDirectoryListing(
...
): Promise<ApiResponse<DirectoryListingContentsResponse>> { // Updated Response Type
    // Default options
    const {
        userId,
        page = 0,
    ...
    try {
        ...
        if (itemTypeFilter === 'folder' || itemTypeFilter === 'all') {
            console.log(`[API Contents] Fetching folders for parent: ${folderId || 'root'}, search: '${searchQuery || ''}'`);
            let folderQuery = supabase
                .from('folders')
                .select('*')
                .eq('user_id', userId); // RLS should handle access, but explicit is safer
        ...
        if (shouldFetchTraces) {
              ...
              console.log(`[API Contents] Fetching traces. Parent: ${folderId || 'root'}, Search: '${searchQuery || ''}', Page: ${page}, Limit: ${limit}`);
              ...
              console.log(`[API Contents] Fetched ${tracesData.length} traces, total count: ${tracesCount}`);
        } else {
