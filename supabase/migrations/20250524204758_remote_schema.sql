...
create policy "Allow anonymous SELECT on public trace blobs"
on "storage"."objects"
as permissive
for select
to anon
using (((bucket_id = 'traces'::text) AND (EXISTS ( SELECT 1
   FROM (traces t
     JOIN trace_permissions tp ON ((t.id = tp.trace_id)))
  WHERE ((t.blob_path = ((objects.bucket_id || '/'::text) || objects.name)) AND (tp.user_id IS NULL))))));


create policy "Allow authenticated users to delete their own objects in traces"
on "storage"."objects"
as permissive
for delete
to authenticated
using (((bucket_id = 'traces'::text) AND (owner = auth.uid())));


create policy "Allow authenticated users to insert into traces bucket"
on "storage"."objects"
as permissive
for insert
to authenticated
with check ((bucket_id = 'traces'::text));


create policy "Allow authenticated users to read from traces bucket"
on "storage"."objects"
as permissive
for select
to authenticated
using ((bucket_id = 'traces'::text));


create policy "Allow authenticated users to update their own objects in traces"
on "storage"."objects"
as permissive
for update
to authenticated
...
create policy "Allow user to read their own snapshots"
on "storage"."objects"
as permissive
for select
to authenticated
using (((bucket_id = 'ai-snapshots'::text) AND (auth.role() = 'authenticated'::text) AND ((auth.uid())::text = split_part(name, '/'::text, 1))));
