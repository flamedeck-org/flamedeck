...
export async function authenticateRequest(
  ...
  try {
    ...
    if (!rpcData || !rpcData.o_is_valid || !rpcData.o_user_id) {
      console.log(
        "Auth Helper: API key is invalid or inactive according to RPC.",
      );
    ...
    if (!hasAllScopes) {
      console.warn(
        `Auth Helper: API Key validated for user ${userIdFromRpc} but missing required scope(s). Required: ${requiredScopes}, Found: ${keyScopes}`,
      );
      return new Response(
        JSON.stringify({ error: "Insufficient permissions for this key" }),
        {
    ...
    console.log(
      `Auth Helper: Authentication successful for user: ${userIdFromRpc}`,
    );
