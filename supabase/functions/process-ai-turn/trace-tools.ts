...
export function executeGetTopFunctions(
  ...
  if (!parseResult.success) {
    console.error(
      "[Tool executeGetTopFunctions] Invalid arguments:",
      parseResult.error,
    );
  ...
  try {
    ...
    const allFrames = frameList.filter((frame) =>
      frame.name !== "[root]" && frame.name !== "(speedscope root)"
    );

    // Sort the filtered frames
    allFrames.sort((a, b) => {
      const weightA = sortBy === "self"
        ? a.getSelfWeight()
        : a.getTotalWeight();
      const weightB = sortBy === "self"
        ? b.getSelfWeight()
        : b.getTotalWeight();
    ...
    const results = topN.map((frame, index) => {
      ...
      const totalPerc = totalNonIdle === 0
        ? 0
        : (100.0 * totalWeight) / totalNonIdle;
      const selfPerc = totalNonIdle === 0
        ? 0
        : (100.0 * selfWeight) / totalNonIdle;
      return `${index + 1}. ${frame.name || "(unknown)"}: ` +
        `Total: ${profile.formatValue(totalWeight)} (${
