  ...
  try {
      if (requestType === 'continue_with_tool_result') {
          ...
          const snapshotToolCall = responseMessage?.tool_calls?.find(
              call => call.type === 'function' && call.function.name === getSnapshotToolSchema.function.name
          );
          ...
          if (snapshotToolCall) {
              ...
              return new Response(JSON.stringify({ success: true, message: "Snapshot requested from client." }), {
                  headers: { ...corsHeaders, "Content-Type": "application/json" },
              ...
              console.log(`[process-ai-turn] Non-snapshot tool call requested:`, responseMessage.tool_calls);
      ...
      console.error(`Error processing request for user ${userId || 'unknown'}, type ${requestType}:`, error);
      // Publish error to Realtime if possible
      if (channel) { try { await channel.send({
          type: 'broadcast', 
          event: 'ai_response', 
          payload: {
              type: 'error',
