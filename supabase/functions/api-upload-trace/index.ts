...
serve(async (req) => {
  ...
  try {
    ...
    try {
      ...
      return new Response(
        JSON.stringify({
          error: "Failed to read trace file from request body",
        }),
        {
          status: 400,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    try {
      console.log(
        `Attempting to import profile using shared importer for file: ${fileName}`,
      );
      ...
      if (!importResult?.profileGroup) {
        ...
        console.warn(
          `Import failed (importer returned null/empty group) for file: ${fileName}`,
        );
        return new Response(
          JSON.stringify({
            error: "Failed to parse trace file or unsupported format",
          }),
          {
            status: 400,
            headers: { ...corsHeaders, "Content-Type": "application/json" },
      ...
      console.log(
        `Extracted from profile - duration: ${durationMs}ms, type: ${profileType}`,
      );
      ...
      console.log(
        `Serializable profile stringified, size: ${uint8Array.byteLength} bytes`,
      );
      ...
      try {
        ...
        console.log(
          `Compression successful, compressed size: ${compressedSize} bytes`,
        );
      } catch (compressError) {
        console.error(
          `Error during compression for ${fileName}:`,
          compressError,
        );
        return new Response(
          JSON.stringify({
            error: "An error occurred while compressing the trace file",
          }),
          {
            status: 500,
            headers: { ...corsHeaders, "Content-Type": "application/json" },
      ...
      console.error(
        `Error during import process for ${fileName}:`,
        importError,
      );
      // Log the error but return a generic message
      return new Response(
        JSON.stringify({
          error: "An error occurred while processing the trace file",
        }),
        {
          status: 500,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    if (compressedBuffer === undefined) {
      console.error(
        "Compression step failed or was skipped, cannot proceed with storage upload.",
      );
      return new Response(
        JSON.stringify({ error: "Internal error during file processing" }),
        {
          status: 500,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        },
    ...
    try {
      console.log(
        `Attempting to upload compressed data to ${bucket}/${filePathInBucket}`,
      );
      ...
      console.error(
        `Error during storage upload for ${fileName}:`,
        uploadCatchError,
      );
      return new Response(
        JSON.stringify({
          error: "An error occurred while uploading the trace file",
        }),
        {
          status: 500,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    try {
      console.log(
        `Attempting to create trace record via RPC for user ${userId}, public: ${isPublic}`,
      );
      const rpcParams = {
        p_user_id: userId,
      ...
      if (rpcError) {
        ...
        console.warn(
          `RPC call failed. Attempting to delete orphaned storage object: ${storagePath}`,
        );
        ...
        if (bucketName && objectPath) {
          ...
          console.log(
            `Cleanup attempt for ${storagePath} finished after RPC error.`,
          );
        } else {
          console.error(
            `Could not parse bucket/path for cleanup after RPC error: ${storagePath}`,
          );
        }
      ...
      console.log(
        `RPC call to create_trace successful. Trace ID: ${insertedTraceData?.id}`,
      );
    } catch (dbProcessingError) { // Catch errors from RPC call or subsequent logic
      console.error(
        `Error during database processing phase (RPC or aftermath) for ${fileName}:`,
        dbProcessingError,
      );
      ...
      if (
        !(dbProcessingError.message.includes(
          "Failed to create trace via RPC",
        )) && storagePath
      ) {
        console.warn(
          `Generic DB processing error. Attempting to delete orphaned storage object: ${storagePath}`,
        );
        const [bucketName, ...objectPathParts] = storagePath.split("/");
        const objectPath = objectPathParts.join("/");
        if (bucketName && objectPath) {
          await supabaseAdmin.storage.from(bucketName).remove([objectPath]);
          console.log(
            `Cleanup attempt for ${storagePath} finished after generic DB error.`,
          );
        } else {
          console.error(
            `Could not parse bucket/path for cleanup after generic DB error: ${storagePath}`,
          );
        }
      }
