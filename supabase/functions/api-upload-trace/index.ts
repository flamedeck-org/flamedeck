...
serve(async (req) => {
  ...
  try {
    ...
    try {
      // Temp: Upload the raw trace for debugging
      // TODO: Add this as a pro feature?
      // try {
      //   const tempFileName = `raw_upload_${crypto.randomUUID()}-${fileName}`;
      //   // Use a debug bucket, ensure fileBuffer is treated as ArrayBuffer by Blob
      //   const rawBlob = new Blob([fileBuffer], { type: 'application/octet-stream' });
      //   await supabaseAdmin.storage.from('traces-raw').upload(tempFileName, rawBlob);
      //   console.log(`Supabase: Saved raw uploaded file to traces-raw/${tempFileName}`);
      // } catch (rawSaveError) {
      //   console.error('Supabase: Error saving raw uploaded file:', rawSaveError);
      // }
    ...
    try {
      ...
      // Catch errors from RPC call or subsequent logic
      console.error(
