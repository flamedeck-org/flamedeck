...
serve(async (req: Request) => {
  ...
  if (!SUPABASE_URL || !SUPABASE_ANON_KEY || !SUPABASE_SERVICE_ROLE_KEY) {
    console.error("Missing Supabase URL, Anon Key, or Service Role Key environment variables for function invocation.");
  ...
  socket.onmessage = async (event) => {
    ...
    try {
      if (parsedData.type === 'start_analysis') {
        ...
        console.log(`[trace-analysis-socket] User prompt received. User: ${userId}, Trace: ${traceId}`);
        
        // Validate required fields from payload
        if (!userId || !traceId || !userPrompt) {
            console.error("[trace-analysis-socket] user_prompt message missing required fields (userId, traceId, prompt).");
            socket.send(JSON.stringify({ type: "error", message: "Internal error: Missing required context in prompt message." }));
            return;
        }
